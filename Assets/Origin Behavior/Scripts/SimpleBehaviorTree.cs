    ...
    public class SimpleBehaviorTree
    {
        ...
        private void PopNode()
        {
            ...
            for (int i = 0; i < _reevaluateList.Count; i++)
            {
                ...
                if(_activeNodeIndex == CommonAncestorNode(cr.Index, _activeNodeIndex))
        ...
        private int ReevaluateConditionalTasks()
        {
            ...
            for (int i = 0; i < _reevaluateList.Count; i++)
            {
                ...
                if (cr.Status != status)
                {
                    ...
                    for (int j = i; j < _reevaluateList.Count; j++)
                    {
                        ConditionalReevaluate cr_after = _reevaluateList[j];
                        _reevaluateList.Remove(cr_after);
                    }
                    //_reevaluateList.Remove(cr);
                    return cr.Index;
                }
        ...
        private BehaviorStatus Execute(Node node)
        {
            ...
            PushNode(node);
            Debug.Log("_reevaluate : "+_reevaluateList.Count +"  nodeindex : "+ node.Index);
